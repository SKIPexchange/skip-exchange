...
export class UserSettingsDialogComponent implements OnInit, OnDestroy {
      ...
      if (user) {
        ...
        if (this.user.clients) {
          ...
          this.ethereumAddress =
            await this.user.clients?.ethereum?.getAddress();
          ...
          this.terraAddress = await this.user.clients?.terra?.getAddress();
        }
  ...
  async getBalances() {
    ...
    const sub = combined.subscribe(([balances, pendingBalances]) => {
